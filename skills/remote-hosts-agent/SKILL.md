...
22. For a `mutating` operation that touches one resource subtree, provide the narrowest stable lowercase `coordination_scope`, for example `k8s/prod/datatool-dev/service/file-gateway`. For one indivisible operation that touches several disjoint resources, omit the singular field and provide all exact resources in `coordination_scopes` (maximum 16); the connector acquires the complete set atomically or acquires none. Do not include a parent and its child in one set, and do not collapse unrelated resources to their common parent. Equal and parent/child scopes across tasks conflict while siblings may mutate concurrently. The Workspace scope is an immutable upper boundary: its default `host` permits any valid operation scope, while a narrow Workspace permits only itself or descendants. Use singular `host` only for genuinely machine-wide or uncertain effects. Never vary spelling, aliases, or scope depth to evade a conflict.
...
26. On cold start, context compression, task recovery, or after any long wait, call `remote_hosts_get_agent_work_context` with `mode=snapshot` before rediscovering Workspace, operation, PTY, transfer, or cursor ids one by one. Retain its `cursor`, follow only its single `primary_action`, and use `mode=wait` with that exact cursor and the same `host_id` filter for subsequent lifecycle changes; start a separate snapshot/wait stream when the Host filter changes. A timeout or `changed=false` is normal and means `overall_state=waiting`, `primary_action.kind=wait`, unchanged cursor, and no delta items; retain the prior changed context and wait again instead of treating the Session as idle or rediscovering work. `lifecycle_outbox.pending > 0` means the secondary admin event stream is degraded, not that terminal work is missing; retain the returned context/cursor and report the bounded age/error without restarting or repeating the mutation. Never execute an action merely because it was returned, and never try to supply an Agent Session id to MCP. Use the older host snapshot and entity-specific reads only when the primary action requests output/runtime inspection or when detailed route/transport evidence is required.
...
29. Idle lifecycle is connector-owned. Ordinary PTYs with no business activity are closed after the configured idle TTL, while zero-channel pooled SSH transports are released after the access path's `idle_ttl_seconds`; SSH keepalive does not count as business activity. For a quiet long-running PTY command, call `remote_hosts_heartbeat_pty_session` with a truthful `foreground_process` before the normal idle TTL and refresh it while the process remains active. Heartbeat is advisory lifecycle metadata, not evidence that the foreground command succeeded or failed. When it returns `metadata_persisted=false`, keep the same PTY and follow `continue_pty_and_retry_heartbeat` after `poll_after_ms`; never signal the remote process, close its PTY, abandon output collection, or start a duplicate command. A local `database is locked`, lifecycle publisher backlog, or metadata warning is not an SSH failure and never authorizes route degradation, reconnect, Connector restart, or mutation replay. Clear the heartbeat or close the PTY when work finishes. Never fake foreground work merely to retain a channel.
...
1. Call `remote_hosts_get_agent_work_context` with `mode=snapshot`. If it returns current work, follow `primary_action` and preserve its `cursor`; do not rediscover ids or create duplicate work.
2. If the context is `idle`, call `remote_hosts_list_hosts` and identify the intended host. For state-only diagnosis, call `remote_hosts_get_host_runtime_snapshot`; inspect `attention` and each access path's `authorized_key_bootstrap`.
3. Call `remote_hosts_prepare_workspace`. Keep its default `host` coordination boundary when this conversation may touch several resources; use a narrower Workspace only when every operation belongs to that subtree. The tool reuses only an `idle` or `working` workspace owned by this Agent Session with the same scope before creating one, automatically closes expired foreign history that owns no queued/running operation or active PTY, and returns compact Agent-session and Workspace identity plus `next_action`. Call the explicit runtime snapshot tool when capacity, transport, attention, or route diagnosis is actually needed. Require `snapshot_version=11` for logical Workspace capacity, transport-runtime identity, SSH channel capacity, per-channel reuse evidence, conversation ownership, exact multi-resource write-lease state, single-hop bastion, managed shell, proactive PTY behavior, and explicit input requests.
4. Call `remote_hosts_run_in_workspace` with `shell.posix` or `shell.powershell`, one script argument, explicit intent, a semantic `idempotency_key`, and realistic timeout/output limits. Set `coordination_mode=read_only` for a fully observational script. Otherwise set `coordination_mode=mutating` and use singular `coordination_scope` for one resource or plural `coordination_scopes` for one indivisible multi-resource action. Set `wait_timeout_ms` for short commands so submission and observing this exact operation are atomic; a narrow profile is a convenience for a matching read-only check.
...
For a typical request like "share Remote Hosts data with another machine":

1. Verify both Remote Hosts API endpoints are intentionally reachable. Use HTTPS for routed or public paths; a private-network HTTP endpoint is acceptable only when explicitly trusted.
2. Configure the peer on each receiving instance with `remote_hosts_configure_instance_sync_peer`, using a pairing token supplied in that credential-only tool. The default `inventory`, `knowledge`, and `credentials` collections synchronize all user-authorized SSH credentials as peer-sealed ciphertext; use an explicit collection list only when the user asks to exclude credentials.
3. Call `remote_hosts_sync_instance_peer` with the configured peer id. Read only its compact sent/applied/duplicate/conflict/rejected result.
4. When `conflicts > 0`, inspect the local conflict record in the admin/API maintenance surface before choosing the authoritative update. Do not use a retry to overwrite a newer local record.
5. Do not turn a sync request into a task migration. Active repositories, PTYs, Workspaces, write leases, and operation queues remain owned by their source instance.
...
- Agent Session ownership, each operation's `requires_write_lease`, exact `coordination_scopes`, legacy summary `coordination_scope`, and overlapping `write_lease.active_leases` when multiple tasks touched the same host;
...
- peer label, sent/applied/duplicate/conflict/rejected counts, and the selected durable collections for an instance synchronization;
