...
22. For `mutating`, provide the narrowest stable lowercase operation `coordination_scope`, for example `k8s/prod/datatool-dev/service/file-gateway`. Equal and parent/child scopes conflict while siblings may mutate concurrently. The Workspace scope is an immutable upper boundary: its default `host` permits any valid operation scope, while a narrow Workspace permits only itself or descendants. Use `host` when work is machine-wide, spans unrelated resources, or has uncertain impact. If one task spans resources, use their real common parent. Never vary spelling, aliases, or scope depth to evade a conflict.
23. Inspect `operation.requires_write_lease`, `operation.coordination_scope`, and `write_lease.active_leases`. Declared `read_only` shell operations do not acquire a write lease and may run concurrently across Agent Sessions within channel capacity. Wait only when a queued mutation overlaps a foreign scope. `recommended_action=wait_for_overlapping_scope_or_refine_scope` permits a narrower scope only when the original scope was genuinely over-broad and no side effect has begun; it never permits inventing a sibling scope.
...
4. Call `remote_hosts_run_in_workspace` with `shell.posix` or `shell.powershell`, one script argument, explicit intent, a semantic `idempotency_key`, and realistic timeout/output limits. Set `coordination_mode=read_only` for a fully observational script. Otherwise set `coordination_mode=mutating` and the narrowest truthful operation `coordination_scope`. Set `wait_timeout_ms` for short commands so submission and observing this exact operation are atomic; a narrow profile is a convenience for a matching read-only check.
...
- Agent Session ownership, each operation's `requires_write_lease` and `coordination_scope`, and overlapping `write_lease.active_leases` when multiple tasks touched the same host;
