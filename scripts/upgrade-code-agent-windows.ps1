param(
  ...
  [string]$BinaryPath="$env:USERPROFILE\.cargo\bin\remote-hosts-code.exe",
  [string]$ConfigPath="$env:USERPROFILE\.local\share\remote-hosts-code\agent.json",
...
try {
  ...
  do {
    ...
    $children=@(Get-CimInstance Win32_Process|Where-Object {$agentIds -contains $_.ParentProcessId})
